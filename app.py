...
# --- CSS PARA MODO OSCURO E INTERFAZ ---
st.markdown("""
    <style>
...
if df is not None:
    ...
    with st.sidebar:
        ...
        orden_musical = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B']
        notas_en_df = df['Raiz'].unique()
        # Filtramos el orden musical para que solo aparezcan las que existen en tu Excel
    ...
    if n_sel:
        ...
        for _, row in df_r[df_r['Naturaleza'].isin(n_sel)].iterrows():
            with st.expander(f"📖 {row['Raiz']} {row['Naturaleza']}", expanded=True):
                ...
                # GALERÍA HORIZONTAL SIN IMÁGENES ROTAS
                imgs = []
                for i in range(1, 10):
                    c = f'Diagrama{i}'
